...
# ══════════════════════════════════════════
# AGENTS INTEGRATION — يستقبل signals من agents ويُنفذ
# ══════════════════════════════════════════

_latest_agent_data = {}
_executed_signals  = {}  # تتبع الصفقات المنفذة لتفادي التكرار

@app.route('/api/agents/update', methods=['POST'])
def api_agents_update():
    """يستقبل signals من agents engine ويُنفذ تلقائياً"""
    global _latest_agent_data
    data = request.json or {}
    _latest_agent_data = data
    ...
    for sig in signals:
        ...
        # تفادي تنفيذ نفس الصفقة مرتين في نفس الدقيقة
        sig_key = f"{pair}_{action}_{timestamp[:16]}"
        if sig_key in _executed_signals:
            continue
        _executed_signals[sig_key] = True

        # تنظيف القاموس من السجلات القديمة
        if len(_executed_signals) > 200:
            keys = list(_executed_signals.keys())
            for k in keys[:100]:
                del _executed_signals[k]

        # تحديد المنصة والحجم
        exchange_id = _get_best_exchange()
        amount_usdt = _get_order_size(pair)

        # جلب السعر الحالي
        price = get_price(pair, exchange_id)

        log.info(f"🎯 SIGNAL: {pair} {action} @ ${price} · conf={confidence:.0f}% · size=${amount_usdt}")

        try:
            client = get_exchange_client(exchange_id)
            if client:
                result = client.place_order(pair, action, amount_usdt, price)
                msg = f"⚡ [SnipBot Execution]: {pair} {action} fired @ ${price}. Confidence: {confidence:.0f}%. Agent: Consensus Engine."
                _send_telegram(msg)
                log.info(f"✅ Order executed: {result}")
        except Exception as e:
            log.warning(f"❌ Order failed: {e}")

    return jsonify({'status': 'received', 'signals_count': len(signals)})

@app.route('/api/agents/status', methods=['GET'])
def api_agents_status_proxy():
    """يمرر status الـ agents للداشبورد"""
    agents_url = os.environ.get('AGENTS_URL', 'https://agents-snipbot.up.railway.app')
    try:
        r = requests.get(f'{agents_url}/api/agents/status', timeout=8)
        return jsonify(r.json())
    except:
        # fallback: آخر بيانات وصلت
        strategies = _latest_agent_data.get('strategies', [])
        return jsonify({
            'status':       'cached',
            'agents':       strategies,
            'scan_count':   _latest_agent_data.get('scan_count', 0),
            'last_scan':    _latest_agent_data.get('timestamp', ''),
            'pairs':        _latest_agent_data.get('symbols', []),
        })

@app.route('/api/agents/evaluate', methods=['GET'])
def api_agents_evaluate():
    """يمرر evaluate لـ agents service"""
    agents_url = os.environ.get('AGENTS_URL', 'https://agents-snipbot.up.railway.app')
    pair = request.args.get('pair', 'ADA/USDT')
    tf   = request.args.get('timeframe', '1h')
    try:
        r = requests.get(f'{agents_url}/api/agents/evaluate?pair={pair}&timeframe={tf}', timeout=15)
        return jsonify(r.json())
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 503

def _get_best_exchange():
    """يختار أفضل منصة متاحة"""
    for ex in ['kucoin', 'binance', 'bybit']:
        cfg = get_exchange_config(ex)
        if cfg['api_key'] and cfg['enabled']:
            return ex
    return 'kucoin'

def _get_order_size(pair):
    """حجم الصفقة من settings أو default"""
    return float(os.environ.get('ORDER_SIZE_USDT', '500'))
